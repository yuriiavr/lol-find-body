...
export default function TFTRooms() {
  ...
  const modes = ['ALL', 'RANKED', 'NORMAL', 'HYPER ROLL', 'DOUBLE UP'];
  ...
  const checkRankRequirement = (roomMinRank: string, roomMaxRank: string) => {
    ...
    const userRankBase = (userProfile.tft_rank || 'Unranked').split(' ')[0].toUpperCase();
  ...
  const handleCreateRoomSubmit = async (description: string, mode: string, maxPlayers: number, minRank: string, maxRank: string) => {
    ...
    const { data, error } = await supabase.from('rooms').insert({ owner_id: user.id, game_type: 'tft', mode, max_players: maxPlayers, min_rank: minRank, max_rank: maxRank, description }).select().single();
