...
export const ProfileSidebar = memo(
  ({
    ...
    const { copied: copiedFriendCode, copy: copyFriendCode } = useCopyToClipboard();
    ...
    const displayTagLine =
      activeGame === "CS2" ? "" : getTagLine(profile, gameKey);
    ...
    const cs2FriendCode =
      activeGame === "CS2" ? (profile?.friend_code ?? "") : "";

    const handleCopy = useCallback(
      () => copy(`${displayGameName}#${displayTagLine}`),
      [displayGameName, displayTagLine, copy],
    );

    const handleCopyFriendCode = useCallback(
      () => copyFriendCode(cs2FriendCode),
      [cs2FriendCode, copyFriendCode],
    );
    ...
    return (
      <section className="w-full lg:w-96 flex flex-col items-center lg:items-start text-center lg:text-left">
        ...
        <div className="flex flex-wrap gap-4 mt-6 justify-center lg:justify-start">
          {displayRole && activeGame !== 'TFT' && activeGame !== 'CS2' && (
            <div className="px-4 py-2 bg-white/5 rounded-full border border-white/5 text-xs font-bold text-[rgb(var(--accent-color))] uppercase tracking-widest flex items-center gap-2">
              <Sword size={14} />{" "}
              {t(`Common.roles.${displayRole.toLowerCase()}`)}
            </div>
          )}
          {language && (
            <div className="px-4 py-2 bg-white/5 rounded-full border border-white/5 text-xs font-bold text-[rgb(var(--accent-color))] uppercase tracking-widest flex items-center gap-2">
        ...
        <div className="mt-10 w-full space-y-4">
          ...
          {activeGame === 'CS2' && cs2FriendCode && isMatched && (
            <button
            ...
            <>
              <RankBox
                ...
                stats={{
                  wins:
                    riotStats?.solo_wins ??
                    getExtra(profile, "lol", "solo_wins") ??
                    0,
                  losses:
                    riotStats?.solo_losses ??
                    getExtra(profile, "lol", "solo_losses") ??
                    0,
                }}
                isMain={true}
              />
              <RankBox
                ...
                rank={
                  riotStats?.flex ||
                  getExtra(profile, "lol", "flex_rank") ||
                  t("ProfilePage.ranks.unranked")
                }
                active={queues.includes("Flex")}
                stats={{
                  wins:
                    riotStats?.flex_wins ??
                    getExtra(profile, "lol", "flex_wins") ??
                    0,
                  losses:
                    riotStats?.flex_losses ??
                    getExtra(profile, "lol", "flex_losses") ??
                    0,
                }}
              />
            </>
          ...
          {activeGame === "TFT" && (
            <RankBox
              ...
              stats={{
                ...
                losses:
                  tftStats?.losses ?? getExtra(profile, "tft", "losses") ?? 0,
              }}
          ...
          {activeGame === "VALORANT" && (
            <RankBox
              ...
              rank={
                valStats?.rankName ||
                getRank(profile, "valorant") ||
                t("ProfilePage.ranks.unranked")
              }
              active={true}
              stats={{
                wins:
                  valStats?.wins ?? getExtra(profile, "valorant", "wins") ?? 0,
                losses:
                  valStats?.losses ??
                  getExtra(profile, "valorant", "losses") ??
                  0,
              }}
          ...
          {activeGame === "CS2" && (
            <RankBox
              ...
              rank={
                getRank(profile, "cs2" as GameKey) ||
                t("ProfilePage.ranks.unranked")
              }
              active={true}
              stats={null}
...
const RankBox = ({
  ...
  return (
    <div
      className={`modern-panel p-5 transition-all ${active ? "bg-[rgb(var(--accent-color)/0.1)] border-[rgb(var(--accent-color)/0.4)]" : "bg-white/5 opacity-60"}`}
    >
      ...
      <p
        className={`${isMain ? "text-3xl" : "text-2xl"} font-bold text-white uppercase italic`}
      >
