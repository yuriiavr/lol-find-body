...
export default function DiscoveryLayout({
  ...
  // Визначаємо, чи ми знаходимося в розділі кімнат (список або конкретна кімната)
  const isRoomSection = pathname.includes('/rooms');
  const isRoomPage = pathname.includes('/room/');

  // Визначаємо активну вкладку лише для сторінок пошуку (Discovery)
  const activeTab = pathname.includes('/tft') 
    ? 'tft' 
    : pathname.includes('/valorant') 
      ? 'valorant' 
      : pathname.includes('/league') ? 'league' : null;

  useEffect(() => {
    if (isRoomSection || isRoomPage || !activeTab) return;

    localStorage.setItem('lastDiscoveryTab', activeTab);
    setActiveGame(activeTab === 'league' ? 'lol' : activeTab as any);
  }, [activeTab, setActiveGame, isRoomSection, isRoomPage]);
