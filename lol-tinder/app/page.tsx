    ...
    <div className="min-h-screen bg-[rgb(var(--bg-primary))] text-slate-50 overflow-x-hidden">
      ...
      <nav className="fixed top-0 w-full z-50 border-b border-white/5 bg-[rgb(var(--bg-secondary)/0.8)] backdrop-blur-lg">
        <div className="max-w-[1600px] mx-auto px-6 h-20 flex justify-between items-center">
          <Link href="/">
            <h1 className="text-2xl font-black bg-gradient-to-r from-[rgb(var(--accent-color))] to-zinc-800 bg-clip-text text-transparent tracking-tighter italic hover:opacity-80 transition-opacity cursor-pointer">LoLMatch</h1>
          </Link>
      ...
      <section className="relative pt-48 pb-32 px-6">
        <div className="max-w-[1200px] mx-auto text-center">
          ...
          >
            <span className="px-4 py-1.5 rounded-full bg-[rgb(var(--accent-color)/0.1)] border border-[rgb(var(--accent-color)/0.2)] text-[rgb(var(--accent-color))] text-[10px] font-black uppercase tracking-[0.3em] mb-10 inline-block">
               Global Duo Finder
            </span>
            <h1 className="text-7xl md:text-9xl font-black tracking-tighter leading-[0.85] uppercase mb-10">
               Stop playing <br/> <span className="bg-gradient-to-r from-[rgb(var(--accent-color))] to-zinc-900 bg-clip-text text-transparent">with randoms.</span>
            </h1>
               ...
               {!loading && (
                 user ? (
                   <div className="flex flex-col sm:flex-row gap-6">
                     <Link href="/league">
                       <button className="btn-modern px-10 py-5 text-lg">
                          <Zap size={20} /> League Discovery
                       </button>
                     </Link>
                     <Link href="/tft">
                       <button className="btn-modern px-10 py-5 text-lg opacity-80 hover:opacity-100">
                          <Gamepad size={20} /> TFT Discovery
                       </button>
                     </Link>
                   </div>
                 ) : (
                   <button onClick={handleLogin} className="btn-modern px-14 py-6 text-xl scale-110">
                      <LogIn size={24} /> Connect via Discord
                   </button>
                 )
               )}
               <Link href="/league" className="group text-lg font-bold flex items-center gap-2 hover:text-[rgb(var(--accent-color))] transition-all">
      ...
      <section className="py-24 px-6 max-w-[1600px] mx-auto">
         <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
            <FeatureCard 
              icon={<Zap size={28} className="text-[rgb(var(--accent-color))]" />}
            ...
            />
            <FeatureCard 
              icon={<Globe size={28} className="text-[rgb(var(--accent-color))]" />}
            ...
            />
            <FeatureCard 
              icon={<Users size={28} className="text-[rgb(var(--accent-color))]" />}
...
function FeatureCard({ icon, title, description }: { icon: React.ReactNode, title: string, description: string }) {
  return (
    <div className="modern-panel p-10 bg-[rgb(var(--bg-secondary)/0.4)] hover:bg-[rgb(var(--bg-secondary)/0.6)]">
